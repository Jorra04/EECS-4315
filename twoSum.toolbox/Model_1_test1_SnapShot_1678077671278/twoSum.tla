...
--algorithm twoSum {
    ...
    
    {
        ...
        (\A y \in 1..Len(input) : 
        (x /= y) /\ (input[x] + input[y] = target) => (((output[1] = input[x]) /\ (output[2] = input[y]))\/((output[1] = input[y]) /\ (output[2] = input[x])))
        ));
    }
}
*)
\* BEGIN TRANSLATION (chksum(pcal) = "62cd1612" /\ chksum(tla) = "d5c01e5b")
...
\* Last modified Sun Mar 05 23:41:04 EST 2023 by jorra04
