...
--algorithm largestInArray {
    ...
    
    {
        ...
        assert (\A a \in 1..Len(input) : output >= input[i]);
    }
}
*)
\* BEGIN TRANSLATION (chksum(pcal) = "f94168c0" /\ chksum(tla) = "3b923216")
...
Lbl_1 == /\ pc = "Lbl_1"
         /\ IF i <= Len(input)
               THEN /\ IF input[i] >= output
                    ...
                    /\ i' = i + 1
                    /\ pc' = "Lbl_1"
               ELSE /\ Assert((\A a \in 1..Len(input) : output >= input[i]), 
...
\* Last modified Sun Apr 02 18:56:24 EDT 2023 by jorra04
