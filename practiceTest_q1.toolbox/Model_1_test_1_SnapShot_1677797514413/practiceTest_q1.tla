...
\*     assert (\A x \in 1..Len(output) : (/\(output[x] \in input1) /\ output[x] /= input2 ) );
        assert (\A x \in 1..Len(output) : (output[x] \in input1));
    }
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "a1d83342" /\ chksum(tla) = "273288d4")
...
Lbl_2 == /\ pc = "Lbl_2"
         /\ IF i <= Len(input1)
               THEN /\ IF input1[i] /= input2
                    ...
                    /\ i' = i + 1
                    /\ pc' = "Lbl_2"
               ELSE /\ Assert((\A x \in 1..Len(output) : (output[x] \in input1)), 
...
\* Last modified Thu Mar 02 17:51:48 EST 2023 by jorra04
