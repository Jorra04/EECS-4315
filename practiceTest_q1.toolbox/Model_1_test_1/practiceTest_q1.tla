...
--algorithm keepAll {
    ...
    assert (\A x \in 1..Len(output) : output[x] /= input2);
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "6b5b65" /\ chksum(tla) = "acb7a75c")
...
Lbl_2 == /\ pc = "Lbl_2"
         /\ IF i <= Len(input1)
               THEN /\ IF input1[i] /= input2
                    ...
                    /\ i' = i + 1
                    /\ pc' = "Lbl_2"
               ELSE /\ Assert((\A x \in 1..Len(output) : output[x] /= input2), 
...
\* Last modified Fri Mar 03 17:43:01 EST 2023 by jorra04
