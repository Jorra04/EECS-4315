...
--algorithm keepAll {
    ...
    assert (\A x \in 1..Len(output) : (output[x] \in Seq(input1) ))
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "e33df752" /\ chksum(tla) = "4ea8a442")
...
Lbl_2 == /\ pc = "Lbl_2"
         /\ IF i <= Len(input1)
               THEN /\ IF input1[i] /= input2
                    ...
                    /\ i' = i + 1
                    /\ pc' = "Lbl_2"
               ELSE /\ Assert((\A x \in 1..Len(output) : (output[x] \in Seq(input1) )), 
...
\* Last modified Thu Mar 02 18:09:04 EST 2023 by jorra04
