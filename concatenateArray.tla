...
--algorithm concatenateArr {
    ...
    
    {
        ...
        (\A b \in (Len(input) + 1)..Len(output) : 
        (IF (b % Len(input)) = 0
        THEN output[b] = input[Len(input)]
        ELSE output[b] = input[b % Len(input)])) ;
        
        
    }
}
*)
\* BEGIN TRANSLATION (chksum(pcal) = "630fac82" /\ chksum(tla) = "cd75e6f3")
...
\* Last modified Sun Apr 02 19:47:53 EDT 2023 by jorra04
