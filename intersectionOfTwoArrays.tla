...
--algorithm intersectionOfArrays {
    variable i = 1, j = 1, output = <<>>, input1Set = {}, input2Set= {}, outputValues = {};
    
    {
        ...
        j := 1;
        while(i <= Len(input1)) {
            if((input1[i] \in input1Set) /\ (input1[i] \in input2Set) /\ ~(input1[i] \in outputValues)) {
                output := Append(output, input1[i]);
                outputValues := outputValues \cup {input1[i]};
            };
            i := i + 1;
        };
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "60f6abde" /\ chksum(tla) = "564b63ac")
VARIABLES i, j, output, input1Set, input2Set, outputValues, pc

vars == << i, j, output, input1Set, input2Set, outputValues, pc >>
...
Lbl_1 == /\ pc = "Lbl_1"
         ...
         /\ UNCHANGED << j, output, input2Set, outputValues >>
...
Lbl_2 == /\ pc = "Lbl_2"
         /\ IF j <= Len(input2)
               ...
               ELSE /\ i' = 1
                    ...
                    /\ pc' = "Lbl_3"
                    /\ UNCHANGED input2Set
         /\ UNCHANGED << output, input1Set, outputValues >>

Lbl_3 == /\ pc = "Lbl_3"
         /\ IF i <= Len(input1)
               THEN /\ IF (input1[i] \in input1Set) /\ (input1[i] \in input2Set) /\ ~(input1[i] \in outputValues)
                          THEN /\ output' = Append(output, input1[i])
                               /\ outputValues' = (outputValues \cup {input1[i]})
                          ELSE /\ TRUE
                               /\ UNCHANGED << output, outputValues >>
                    /\ i' = i + 1
                    /\ pc' = "Lbl_3"
               ELSE /\ Assert(Len(output) <= Len(input1) /\ Len(output) <= Len(input2), 
                    ...
                    /\ pc' = "Done"
                    /\ UNCHANGED << i, output, outputValues >>
...
\* Last modified Mon Apr 03 18:22:54 EDT 2023 by jorra04
