...
--algorithm findDuplicate {
    ...
    
    {
        ...
        assert output # -1 => (\E a,b \in 1..Len(input) : (i # j)/\(input[i] = input[j])/\( (input[i] = output) \/ (input[j] = output) ));
    }
}
*)
\* BEGIN TRANSLATION (chksum(pcal) = "ec04df76" /\ chksum(tla) = "708243ae")
...
\* Last modified Sun Apr 02 21:50:17 EDT 2023 by jorra04
