...
\*            )
            ...
            assert (\A x \in 1..Len(input) :
               IF( ((((x- numShifts) % Len(output)) + Len(output)) % Len(output)) = 0 )
               THEN input[x] = output[Len(output)]
               ELSE input[x] = output[((((x-numShifts) % Len(output)) + Len(output)) % Len(output))]
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "8644475d" /\ chksum(tla) = "36e86822")
...
\* Last modified Mon Mar 06 13:22:06 EST 2023 by jorra04
