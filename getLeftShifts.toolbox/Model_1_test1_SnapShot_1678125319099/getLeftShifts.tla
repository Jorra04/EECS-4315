...
(*
    --algorithm getLeftShift {
        ...
        
        {
            ...
            IF(((x + (numShifts * Len(input))) % Len(input)) = 0)
            THEN input[x] = output[Len(output)]
            ELSE input[x] = output[(numShifts * Len(input)) % Len(input)]
            )
        }
    }
*)
\* BEGIN TRANSLATION (chksum(pcal) = "c9c5038c" /\ chksum(tla) = "fa17039b")
...
\* Last modified Mon Mar 06 12:55:09 EST 2023 by jorra04
