...
\*            )
            ...
            assert (\A x \in 1..Len(input) :
               IF( ((((x-1) % Len(output)) + Len(output)) % Len(output)) = 0 )
               THEN input[x] = output[Len(output)]
               ELSE input[x] = output[((((x-1) % Len(output)) + Len(output)) % Len(output))]
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "ce4e635a" /\ chksum(tla) = "df708e64")
...
\* Last modified Mon Mar 06 13:17:51 EST 2023 by jorra04
