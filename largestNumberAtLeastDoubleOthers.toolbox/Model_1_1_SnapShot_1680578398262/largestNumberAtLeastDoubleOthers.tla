...
--algorithm largestAtLeastDouble {
    ...
    
    {
        ...
        assert output = TRUE <=> (\E x \in 1..Len(input) : (\A y \in 1..Len(input) : (input[x] >= 2 * input[y]) /\ (x # y)  ));
    }
}
*)
\* BEGIN TRANSLATION (chksum(pcal) = "6464a98" /\ chksum(tla) = "272e3a8a")
...
Lbl_3 == /\ pc = "Lbl_3"
         /\ IF i <= Len(input)
               THEN /\ IF ~(max >= 2 * input[i]) /\ (i # indexFound)
                    ...
                    /\ i' = i + 1
                    /\ pc' = "Lbl_3"
               ELSE /\ Assert(output = TRUE <=> (\E x \in 1..Len(input) : (\A y \in 1..Len(input) : (input[x] >= 2 * input[y]) /\ (x # y)  )), 
...
\* Last modified Mon Apr 03 23:19:39 EDT 2023 by jorra04
