...
--algorithm getAllSuffixes {
    variable result = <<>>, subresult = "", i = 1, j = 1;
    {
        ...
        assert /\ (Len(input) = Len(result)) 
                /\ (\A k \in 1..Len(result)-1 : Len(result[k]) = 1 + Len(result[k+1]));
    }
}
*)
\* BEGIN TRANSLATION (chksum(pcal) = "e15956c8" /\ chksum(tla) = "7b383df9")
...
\* Last modified Fri Feb 24 04:41:25 EST 2023 by jorra04
