...
\*        assert /\ ((Len(array) = bound) /\ 
\*        (\A X \IN 1..LEN(ARRAY) -1 : ARRAY[X] = (ARRAY[X] + 1) ));
        assert (Len(array) = bound);
        assert (\A x \in 1..Len(array)-1 : array[x] = array[x+1] - 1);
    }
}
*)
\* BEGIN TRANSLATION (chksum(pcal) = "98b5ba16" /\ chksum(tla) = "b4424d7a")
...
\* Last modified Thu Mar 02 11:45:09 EST 2023 by jorra04
