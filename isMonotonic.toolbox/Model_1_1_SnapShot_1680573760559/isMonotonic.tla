...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "16a1c004" /\ chksum(tla) = "66b58d22")
...
Lbl_1 == /\ pc = "Lbl_1"
         /\ IF i < Len(input)
               ...
               ELSE /\ Assert(       output = TRUE <=> (\A x \in 1..(Len(input)-1) : input[x] <= input[x + 1]) \/
                              (\A x \in 1..(Len(input)-1) : input[x] >= input[x + 1]), 
                              "Failure of assertion at line 21, column 11.")
                    /\ pc' = "Done"
...
\* Last modified Mon Apr 03 22:00:59 EDT 2023 by jorra04
