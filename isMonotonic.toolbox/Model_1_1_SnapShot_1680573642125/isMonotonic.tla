...
\*        assert ~(isIncreasing /\ isDecreasing);
\*        assert isIncreasing <=> (\A x \in 1..(Len(input)-1) : input[x] <= input[x + 1]);
\*        assert isDecreasing <=> (\A x \in 1..(Len(input)-1) : input[x] >= input[x + 1]);
...
*)
\* BEGIN TRANSLATION (chksum(pcal) = "2acbeb9" /\ chksum(tla) = "1c7ba036")
...
Lbl_1 == /\ pc = "Lbl_1"
         ...
         /\ Assert(       output' = TRUE <=> (\A x \in 1..(Len(input)-1) : input[x] <= input[x + 1]) \/
                   (\A x \in 1..(Len(input)-1) : input[x] >= input[x + 1]), 
                   "Failure of assertion at line 23, column 11.")
         /\ pc' = "Done"
         /\ UNCHANGED << i, isIncreasing, isDecreasing >>
...
\* Last modified Mon Apr 03 22:00:28 EDT 2023 by jorra04
