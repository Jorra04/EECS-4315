...
\*         );
        ...
        assert (\A x \in 1..Len(input) : 
        (((x = max) => (\A y \in 1..Len(input) : (x # y) => (x > y) ))/\((x = output) => (\A y \in 1..Len(input): (x # y) => ((x > y)/\ x < max)))));
        
    }
}
*)
\* BEGIN TRANSLATION (chksum(pcal) = "8593381c" /\ chksum(tla) = "af42608d")
...
\* Last modified Mon Mar 06 01:25:21 EST 2023 by jorra04
