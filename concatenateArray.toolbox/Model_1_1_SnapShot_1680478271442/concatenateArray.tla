...
--algorithm concatenateArr {
    ...
    
    {
        ...
        assert (\A a \in 1..Len(input) : output[a] = input[a]) /\ (\A b \in Len(input)+1..Len(output) : output[b] = input[(b % Len(input)) + 1]);
    }
}
*)
\* BEGIN TRANSLATION (chksum(pcal) = "9b3a4d10" /\ chksum(tla) = "4dba05bf")
...
\* Last modified Sun Apr 02 19:31:06 EDT 2023 by jorra04
