...
\*            (input[k] % 3 = 0) => ((\A l \in 1..k) : (input[l] % 3 = 0))
\*            ); 
assert (\A k \in 1..Len(output) :
            (output[k] % 3 = 0) => ((\A l \in 1..k : output[l] % 3 = 0))
            );         
    }
}
*)
\* BEGIN TRANSLATION (chksum(pcal) = "7a1aaa94" /\ chksum(tla) = "38b3ba4")
...
Lbl_4 == /\ pc = "Lbl_4"
         /\ IF j <= Len(temp2)
               THEN /\ output' = output \o <<temp2[j]>>
                    /\ j' = j + 1
                    /\ pc' = "Lbl_4"
               ELSE /\ Assert((\A k \in 1..Len(output) :
                                   (output[k] % 3 = 0) => ((\A l \in 1..k : output[l] % 3 = 0))
                                   ), 
                              "Failure of assertion at line 57, column 1.")
                    /\ pc' = "Done"
                    /\ UNCHANGED << output, j >>
         /\ UNCHANGED << numberOfMultiplesOf3, numberOfMultiplesOf3Plus1, 
                         numberOfMultiplesOf3Plus2, temp0, temp1, temp2, i, 
...
\* Last modified Thu Mar 30 17:42:25 EDT 2023 by jorra04
